...
namespace flatlib
{
    ...
    struct flat_view<Range> : std::ranges::view_interface<flat_view<Range>>
    {
        ...
        // Iter base() const { return *static_cast<details::wrap_if_pointer_t<Iter> const*>(this); }
        ...
        constexpr flat_view& operator=(flat_view &&) noexcept = default;
				...
				auto& operator--() requires std::bidirectional_iterator<underlying_iterator>
				{
					--m_begin;
					return *this;
				}

				auto operator--(int) requires std::bidirectional_iterator<underlying_iterator>
				{
					auto copy = *this;
					this->operator--();
					return copy;
				...
				{
					return (m_begin == m_end && other.m_begin == other.m_end) || m_begin == other.m_begin;
				}

				auto operator<=>(flat_view const& other) const requires std::random_access_iterator<underlying_iterator>
				{
					return m_begin <=> other.m_begin;
				}

				difference_type operator-(flat_view const& other) const requires std::random_access_iterator<underlying_iterator>
				{
					return m_begin - other.m_begin;
				}

				auto& operator+=(difference_type n) requires std::random_access_iterator<underlying_iterator>
				{
					m_begin += n;
					return *this;
				}

				auto operator+(difference_type n) const requires std::random_access_iterator<underlying_iterator>
				{
					auto copy = *this;
					copy += n;
					return copy;
				...
				friend auto operator+(difference_type n, flat_view view) requires std::random_access_iterator<underlying_iterator>
				{
					return view + n;
				}
				...
				reference operator[](difference_type n) const requires std::random_access_iterator<underlying_iterator>
				{
					return *(m_begin + n);
				}
    };
    ...
    struct flat_view<Range> : std::ranges::view_interface<flat_view<Range>>
    {
        ...
        using subrange = flat_view<std::ranges::range_value_t<Range>>;
				...
				// Maybe flat_view of compile time known sizes: like std:;array<std::array<int, N>, M> will be random access?
				using iterator_category = std::conditional_t<
					std::is_same_v<typename subrange::iterator_category, std::input_iterator_tag>,
        ...
        flat_view& operator++() {
            ...
            return *this;
				...
				flat_view& operator--() requires (std::bidirectional_iterator<range_iterator> && std::bidirectional_iterator<std::ranges::iterator_t<subrange>>)
					= delete; // TODO
        ...
        bool operator==(flat_view const& other) const noexcept
        {
            ...
            if (m_range_begin) {
                ...
                return *m_range_begin == m_range_end || *m_range_begin == other.m_range_end;
            }
...
static_assert(std::forward_iterator<flatlib::flat_view<std::forward_list<int>>>);
static_assert(std::bidirectional_iterator<flatlib::flat_view<std::list<int>>>);
static_assert(std::random_access_iterator<flatlib::flat_view<std::vector<int>>>);

static_assert(std::forward_iterator<flatlib::flat_view<std::forward_list<std::vector<int>>>>);
static_assert(std::forward_iterator<flatlib::flat_view<std::list<std::vector<int>>>>);

static_assert(std::same_as<flatlib::flat_view<std::vector<std::vector<int>>>::iterator_category, std::forward_iterator_tag>);
...
int main()
{
		test([](std::stringstream &out)
		{
			std::stringstream ss1, ss2;
			ss1 << "123";
			ss2 << "456";

			auto list_of_stream_iterators = std::forward_list<std::ranges::subrange<std::istream_iterator<char>, std::istream_iterator<char>>>{
					{ std::istream_iterator<char>{ss1}, std::istream_iterator<char>{} },
					{ std::istream_iterator<char>{ss2}, std::istream_iterator<char>{} }
			};

			for (auto &el : flatlib::flat(list_of_stream_iterators)) { out << el; }
		}, "123456");

		test([](std::stringstream &out)
		{
			std::vector<std::array<int, 3>> colors = { { 0xff, 0xff, 0xff  }, { 0, 0, 0 } };
			for (auto &el : flatlib::flat(colors)) { out << el; }
		}, "255255255000");
